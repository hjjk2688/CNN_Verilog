...
### 3.1 데이터 처리 흐름: "저장 후 처리" vs "실시간 스트리밍" 
...
### 3.2 연산 구조 및 속도: "직렬 순차 처리" vs "병렬 트리 처리"
...
### 3.3 양자화 전략: "수학적 정밀도" vs "시스템 인지 최적화" 
...
## 4. 전체 시스템 구조 (Overall System Architecture)
...
### 4.2 AI 모델 아키텍처 (Model Structure)
...
## 5. 하드웨어 설계 (Hardware Design)

### 5.1 클럭 아키텍처 및 CDC 설계
...
## 6. CNN 가속기 상세 설계 (Detailed Design)
...
### 6.1 시스템 인지 양자화(하드웨어 친화적 양자화): 고정 스케일링 (Fixed Scaling)
...
### 6.3 합성곱 연산 계층: 극단적 병렬화

합성곱 계층은 전체 연산의 병목 구간이므로 **'속도 최우선'** 설계를 적용했습니다.

#### 6.3.1 라인 버퍼 (Line Buffer)
...
#### 6.2.2 커널 병렬성(Kernel Parallelism)
...
#### 6.3.3 이진 덧셈 트리 (Binary Adder Tree)
...
#### 6.2.4 다채널 처리(Multi-Channel Processing)
...
## 6.5 VGA 디스플레이 컨트롤러 설계 (VGA Display Controller)
...
## 7. 소프트웨어 설계 및 학습 (Software & Training)
...
### 8.1 성능 측정 결과 (Performance Measurement)
...
### 8.2 아키텍처 효율성 분석 (Architectural Efficiency)
